...
#### Org. Link: https://github.com/3DTopia/LGM/blob/main/mvdream/mv_unet.py
...
def get_camera(
    ...
    print(torch.from_numpy(np.stack(cameras, axis=0)).float())
...
class BasicTransformerBlock3D(nn.Module):
    ...
    def forward(self, x, context=None, num_frames=1):
        x = rearrange(x, "(b f) l c -> b (f l) c", f=num_frames).contiguous() # (batch x frames) x (height * width) x channels ---> batch x (frames x height x width) x channels # self attn: share information across multiviews
        x = self.attn1(self.norm1(x), context=None) + x
        x = rearrange(x, "b (f l) c -> (b f) l c", f=num_frames).contiguous() # batch x (frames x height x width) x channels ---> (batch x frames) x (height * width) x channels # cross attn: all views receive equally same multi-modal information on its spatial frame
